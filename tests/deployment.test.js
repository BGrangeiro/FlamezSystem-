...
test('HTTP: login, proteção de API, CSRF, limites, dados privados e logout',async()=>{
  ...
  try {
    ...
    assert.deepEqual(presets.rows.map(r=>r.data.Código).sort(),['01-1','01-2']);
    assert.equal(presets.rows[0].data.Horas,'1.25');
    const materialResponse=await post('/api/sheets/upsert',{sheet:'materialStock',data:{Material:'Argolas',Quantidade:'12',Unidade:'un','Link de compra':'https://example.com/argolas'}});
    assert.equal(materialResponse.status,200);
    const material=await materialResponse.json();
    await fetch(base+'/api/sheets?sheet=maquinas',{headers});
    const savedMaterials=await (await fetch(base+'/api/sheets?sheet=materialStock',{headers})).json();
    assert.equal(savedMaterials.rows[0].data.Quantidade,'12');
    assert.equal(savedMaterials.rows[0].data['Link de compra'],'https://example.com/argolas');
    assert.equal((await post('/api/sheets/delete',{sheet:'materialStock',rowNumber:material.rowNumber})).status,200);
    assert.equal((await (await fetch(base+'/api/sheets?sheet=materialStock',{headers})).json()).rows.length,0);
