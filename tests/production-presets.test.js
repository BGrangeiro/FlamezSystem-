...
const input={'Produto ID':'2',Quantidade:'12',Horas:'2,5','Filamento (g)':'300,5'};
test('padrões recebem sequência por SKU e mantêm código na edição',()=>{
 ...
 assert.deepEqual(presetValues({data:edited}),{productId:'2',quantity:'24',hours:'2.50',filament:'300.5'});
 assert.equal(first.Quantidade,'12');
 assert.equal(first['Filamento (g)'],'300.5');
});
test('padrões rejeitam produto inválido, quantidades fracionadas e horas inválidas',()=>{
 for(const delta of [{'Produto ID':'404'},{Quantidade:'1.5'},{Quantidade:'0'},{Horas:'-1'},{Horas:'0.001'},{'Filamento (g)':'0'},{'Filamento (g)':'inválido'}])assert.throws(()=>normalizePreset({...input,...delta},products,[]));
