...
export function applyProductionOutcome(items, status) {
  ...
  return items.map(item => {
    ...
    const waste = status === 'Falhou' ? weight : status === 'Parcial' ? machineNumber(item.waste) : 0;
    if (!Number.isFinite(waste) || waste < 0 || waste > weight) throw new Error('O desperdício parcial deve estar entre zero e o filamento total do item.');
    ...
    return {...item, plannedUsed: weight, plannedFilamentTotal: baseCost, plannedHours,
      used: weight - waste, waste, total: baseCost, hours,
