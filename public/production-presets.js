...
export const PRESET_HEADERS=['Código','Produto','SKU','Produto ID','Quantidade','Horas'];
export function normalizePreset(data,products,rows,previous) {
  ...
  const quantity=machineNumber(data.Quantidade),hours=machineNumber(data.Horas);
  ...
  return {'Código':code,Produto:product.data.Produto,SKU:sku,'Produto ID':String(product.rowNumber),Quantidade:String(quantity),Horas:hours.toFixed(2)};
}
export function presetValues(preset) {
  return {productId:String(preset.data['Produto ID']),quantity:preset.data.Quantidade,hours:Number(preset.data.Horas).toFixed(2)};
}
export function renderProductionPresets(ctx) {
  ...
  elements.content.replaceChildren(el('p','machine-formula','Crie versões do mesmo SKU com quantidade e tempo total de impressão. Exemplo: A01-1, A01-2. Use esses padrões em Completar dados da produção.'));
  ...
  for(const row of state.draft?[state.draft,...state.rows]:state.rows){
    ...
    if(row.rowNumber && !state.editingPresets.has(String(row.rowNumber))){
      card.append(el('p','',row.data.Produto),el('strong','',`${row.data.Quantidade} un · ${Number(row.data.Horas).toFixed(2)} h`),button('Editar padrão',()=>{state.editingPresets.add(String(row.rowNumber));redraw();}));continue;
    }
    ...
    for(const [title,input] of [['Produto / SKU',select],['Quantidade produzida (un)',quantity],['Tempo total na máquina (h)',hours]]){const label=el('label','field',title);label.append(input);form.append(label);}
    ...
    form.onsubmit=async e=>{e.preventDefault();try{const data=normalizePreset({'Produto ID':select.value,Quantidade:quantity.value,Horas:hours.value},state.presetProducts,state.rows,row.rowNumber?row.data:null);await saveRow('productionPresets',row.rowNumber,data,save);}catch(e){error.textContent=e.message;}};
  }
