...
export function renderProduction(ctx) {
  const {state, elements, escapeHtml: esc, formatMoney: money, formatNumber: num, formatDateDisplay: date, productKey, render, saveRow, deleteRow, todayInputValue} = ctx;
  ...
  groups.forEach(group => {
    ...
    const breakdown = (label, value, costs = false) => {
      ...
      head.append(node('span','cell-label',label), node('strong','cell-value',value), node('span','metric-arrow','⌄'));
      details.append(head);
    ...
    const addCell = node('div', 'production-day-action'); addCell.append(add); summary.append(addCell);
    ...
    if (!draft && !state.editingProduction.has(key)) {
      ...
      if (items) items.forEach(item => {
        ...
        detail.append(node('h4','production-item-title',item.name || item.sku || 'Produção sem nome'));
        const body = node('div','production-detail-body');
        if(item.sku) body.append(node('strong','production-detail-sku',item.sku));
        const materials = Array.isArray(item.materials) ? item.materials : [];
        const filament = item.filamentLabel || materials.map(m=>m.name).filter(Boolean).join(' + ') || 'Filamento não informado';
        body.append(node('p','production-detail-quantity',`${num(item.quantity,0)} un · ${filament} · ${num(item.used + item.waste)} g`));
        body.append(node('span',`order-status ${savedStatus === 'Concluída' ? 'done' : savedStatus === 'Falhou' ? 'stopped' : 'progress'}`,savedStatus));
        ...
        body.append(calculation);detail.append(body);
        grid.append(detail);
      });
      ...
      panel.append(button('Fazer alterações',()=>{state.editingProduction.add(key);render();}));
      return;
    }
    ...
    const outcomeFields = (entry, snapshot) => {
      ...
      const filament = field('Filamento usado (material e cor)','text',snapshot?.filamentLabel || '');
      filament.input.placeholder = 'Ex.: PETG BRANCO · vazio usa os materiais do produto';
      entry.filament = filament.input;
      entry.wrap.append(filament.wrap,waste.wrap,failure.wrap);
    };
    const readOutcomes = () => applyProductionOutcome(entries.map(entry=>({...readEntry(entry), filamentLabel: entry.filament.value.trim(), waste: status.value === 'Parcial' ? entry.waste.value : 0, failureHours: entry.failure.value})),status.value);
    const refresh=()=>{
    ...
    const addItem = snapshot => {
      ...
      wrap.append(label,quantity.wrap,machineLabel,hours.wrap,button('Remover',()=>{entries.splice(entries.indexOf(entry),1);wrap.remove();refresh();},'danger-button'));
    ...
    const addManual = snapshot => {
      ...
      wrap.append(name.wrap,grams.wrap,price.wrap,label,hours.wrap,button('Remover',()=>{entries.splice(entries.indexOf(entry),1);wrap.remove();refresh();},'danger-button'));
      wrap.addEventListener('input',refresh); list.append(wrap); refresh();
    };
    ...
    if(!draft) actions.append(button('Excluir',()=>deleteRow('producao',row.rowNumber),'danger-button'));
