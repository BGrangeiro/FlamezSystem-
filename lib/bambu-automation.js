...
function syncAutomaticProduction(sheets, device, machine, job) {
  ...
  const selectedProduct = sheets.produtos.rows.find(candidate => String(candidate.rowNumber) === String(previousItem.productRow));
  if (selectedProduct) Object.assign(product, {
    productRow: selectedProduct.rowNumber, sku: String(selectedProduct.data.SKU || '').trim().toUpperCase(),
    name: String(selectedProduct.data.Produto || '').trim(), recognized: true
  });
  const item = {
    ...previousItem,
  ...
  if (item.plannedUsed > 0) Object.assign(item, applyProductionOutcome([item],status)[0]);
  Object.assign(row.data, {
    ...
    'Peso (g)': previousItem.plannedUsed ? String(item.used + item.waste) : '',
    'Horas (h)': String(hours), 'Desperdício (g)': previousItem.plannedUsed ? String(item.waste) : '',
    'Filamento usado': previousItem.filamentLabel || '', 'Custo do filamento (R$)': previousItem.plannedUsed ? String(item.total) : '',
...
export function bindProduction(sheets, row, previous, at = Date.now()) {
  ...
  if (previous && (previous.data._bambuManualOutcome === 'true' ||
...
export function completeCloudProductions(sheets, onlyRow) {
  ...
  for (const row of sheets.producao.rows) {
    ...
    const next = items.map((item, index) => {
      ...
      const itemStatus = jobs[index].state === 'FAILED' ? 'Falhou' : 'Concluída';
      return applyProductionOutcome([{ ...item, plannedHours: hours, failureHours: hours }], itemStatus)[0];
    });
    row.data['Itens da produção'] = JSON.stringify(next);
    row.data['Status da produção'] = status; row.data._bambuSyncError = ''; row.data._bambuAutomaticOutcome = 'true';
