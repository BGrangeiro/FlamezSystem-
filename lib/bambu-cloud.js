...
export function createBambuCloud({ request = fetch, connectMqtt, now = Date.now, schedule = setInterval, cancelSchedule = clearInterval, sessionStore, usage, onInventory, onReport } = {}) {
  ...
  async function startSession(token, username, data, restored = false) {
      const connect = connectMqtt || (await import('mqtt')).connect;
