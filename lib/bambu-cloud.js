import { identifiedModel, identifiedName } from './bambu-model.js';
...
const textValue = (value, max = 80) => typeof value === 'string' && value.trim() ? value.trim().slice(0, max) : null;
function mergeTelemetry(previous = {}, report) {
  const next = { ...previous, temperatures: { ...(previous.temperatures || {}) }, fans: { ...(previous.fans || {}) } };
  const temperatures = {
    nozzle: ['nozzle_temper', -20, 500], nozzleTarget: ['nozzle_target_temper', 0, 500],
    bed: ['bed_temper', -20, 200], bedTarget: ['bed_target_temper', 0, 200],
    chamber: ['chamber_temper', -20, 150], chamberTarget: ['chamber_target_temper', 0, 150]
  };
  for (const [name, [field, min, max]] of Object.entries(temperatures)) if (field in report) {
    const value = numeric(report[field], min, max); if (value !== null) next.temperatures[name] = value;
  }
  const simple = {
    currentLayer: ['layer_num', 0, 1000000], totalLayers: ['total_layer_num', 0, 1000000],
    stage: ['mc_print_stage', 0, 1000], speedLevel: ['spd_lvl', 0, 20],
    wifiSignal: ['wifi_signal', -200, 100], nozzleDiameter: ['nozzle_diameter', 0.1, 2]
  };
  for (const [name, [field, min, max]] of Object.entries(simple)) if (field in report) {
    const value = numeric(report[field], min, max); if (value !== null) next[name] = value;
  }
  const fans = { part: 'cooling_fan_speed', hotend: 'heatbreak_fan_speed', auxiliary: 'big_fan1_speed', chamber: 'big_fan2_speed' };
  for (const [name, field] of Object.entries(fans)) if (field in report) {
    const value = numeric(report[field], 0, 100000); if (value !== null) next.fans[name] = value;
  }
  const amsReport = Array.isArray(report.ams) ? report.ams : report.ams?.ams;
  const activeTray = report.tray_now ?? report.ams?.tray_now;
  if (activeTray != null) next.activeTray = textValue(String(activeTray), 16);
  if (Array.isArray(amsReport)) {
    next.ams = amsReport.slice(0, 8).map(unit => ({
      id: textValue(String(unit?.id ?? ''), 16), humidity: numeric(unit?.humidity, 0, 100), temperature: numeric(unit?.temp, -20, 100),
      trays: Array.isArray(unit?.tray) ? unit.tray.slice(0, 8).map(tray => ({ id: textValue(String(tray?.id ?? ''), 16), type: textValue(tray?.tray_type), color: textValue(tray?.tray_color, 16), remaining: numeric(tray?.remain, 0, 100) })).filter(tray => tray.id || tray.type || tray.color) : []
    }));
  }
  // Presence bits are sent independently from the detailed AMS inventory.
  const bits = report.ams?.ams_exist_bits ?? report.ams_exist_bits;
  const mask = typeof bits === 'number' && Number.isSafeInteger(bits) && bits >= 0 ? bits
    : typeof bits === 'string' && /^(?:0x)?[0-9a-f]{1,8}$/i.test(bits.trim()) ? parseInt(bits.trim().replace(/^0x/i, ''), 16) : null;
  if (mask !== null) next.hasAms = mask > 0;
  else if (Array.isArray(amsReport)) {
    if (next.ams.some(unit => unit.id != null && unit.id !== '')) next.hasAms = true;
    else if (!amsReport.length) next.hasAms = false;
  }
  return next;
}
...
export function mergeReport(previous, payload, now = Date.now()) {
  ...
  let received = false;
  const telemetry = mergeTelemetry(next.telemetry, report);
  if (JSON.stringify(telemetry) !== JSON.stringify(next.telemetry || {})) { next.telemetry = telemetry; received = true; }
...
export function createBambuCloud({ request = fetch, connectMqtt, loadMqtt = () => import('mqtt'), now = Date.now, schedule = setInterval, cancelSchedule = clearInterval, sessionStore, usage, onInventory, onReport } = {}) {
  ...
  return {
    ...
    status() {
      return { authenticated, connected, message, discoveryMessage, inventoryUpdatedAt, storageError: usage?.error || automationError, devices: devices.map(device => ({ ...device, model: identifiedModel(device), name: identifiedName(device),
