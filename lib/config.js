...
export function loadConfig(env = process.env) {
  ...
  if (!['json', 'sqlite'].includes(storageBackend)) throw new Error('STORAGE_BACKEND deve ser json ou sqlite.');
  ...
  return {root, production, port, host, dataDir, storageBackend, username, passwordHash, origin, deletionPassword,autoLoginByIP,trustedProxyIPs:trustedProxyIPs.map(normalizeIP),backupHours,backupKeep,backupDir};
}
